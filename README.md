...
- **Best guess:** We pick the single combo with highest backtest return among those that were profitable (and had at least one trade). That’s our best guess given the data—the reported return may be partly luck. The other profitable combos could have real edge too but got a lower return in that window simply by being unlucky. Either way, that doesn’t change the *direction*: we’re still doing RSI mean reversion (buy oversold, sell on recovery) with params in a sensible band, so the strategy is directionally sound even if the exact “best” is partly lucky; the main uncertainty is how much return to expect, not whether we’re betting the right way. A reasonable range to expect is from 0% (regime shifts against us or bad luck) up to the backtest return; in practice, out-of-sample results often fall below the in-sample best, so the true expected return may sit somewhere in the lower half of that range.
