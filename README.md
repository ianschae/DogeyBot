...
- **Best of many:** We assume that selecting the single best-performing combo from a set of *already profitable* options (rather than a random one) increases the likelihood that the chosen strategy has genuine directional edge, not just backtest luck.
...
- **Regime persistence:** We assume that recent history (the last 350 candles) is a reasonable proxy for the regime we'll face in the near term—i.e. that volatility and mean-reversion behavior are somewhat persistent, so what worked best on that window is a plausible candidate going forward.
- **Regime recurrence:** The backtest window may be a one-off favorable slice or a regime that recurs. We assume that re-running the backtest periodically (e.g. every 24h) on rolling history makes it likely we will repeatedly find at least some profitable param sets when similar conditions appear, and that re-learning lets the bot adapt to regime changes by re-selecting the best combo on the most recent window.
- **Closed-candle alignment:** We assume that trading only on closed candles (same as the backtest) keeps live and backtest directly comparable and avoids strategy drift from intrabar or tick data.
- **Param stability:** The search grid is bounded (entry 10–45, exit 41–76, fixed RSI(14) mean-reversion rule), so the “best” combo stays in an interpretable RSI range (e.g. entry in oversold, exit in recovery). We assume that when the best params change, they will typically move within that band, so updates are incremental and do not cause large, discontinuous changes in trading behavior.
